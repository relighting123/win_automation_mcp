...
# MODE_INSTRUCTIONS
## manual
당신은 [수동 실행 모드]입니다. 제공된 '현재 스킬의 도구 순서(tool_sequence)'를 절대 변경하거나 생략하지 마세요. 리스트에 있는 모든 도구에 대해 하나도 빠짐없이 순서대로 파라미터를 추출해야 합니다. 질문에 관련 내용이 없더라도 기본값을 사용하여 모든 단계를 포함하세요.
