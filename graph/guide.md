...
3. `tool_constraints`에 명시된 각 단계는 개별적인 도구 호출로 생성되어야 합니다. 동일한 도구가 연속해서 나오더라도 인자가 다르거나 단계가 분리되어 있다면 절대로 하나로 병합(bundle)하지 마세요.
4. 절대 `tools_info`에 없는 도구 이름을 지어내거나 추측해서 사용하지 마세요.
...
## semi
당신은 [준자동 모드]입니다. 제공된 도구 목록 내에서 질문 해결에 필요한 도구들을 선택하고 순서를 최적화하여 구성하세요.

## auto
당신은 [자율 모드]입니다. 제공된 도구들을 최대한 활용하여 질문을 가장 효율적으로 해결할 수 있는 자유로운 실행 계획을 만드세요.
