...
class NormalizeMcpToolResultTest(unittest.TestCase):
    def test_is_error_with_text_is_failure(self) -> None:
        ...
        self.assertIn("No connection", normalized["message"])
...
class SequenceSkillBrowserStepTest(unittest.IsolatedAsyncioTestCase):
    def _browser_skill(self) -> SequenceSkill:
        ...
        skill.steps = [
            {"tool": "browsermcp/browser_navigate", "args": {"url": {"mode": "ai"}}},
            {"tool": "browsermcp/browser_snapshot", "args": {}},
        ]
        skill.description = ""
        skill.instruction = ""
        return skill
    ...
    def test_missing_required_url_raises_clear_error(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "필수 인자가 없습니다: url"):
            skill._validate_parsed_step(
                skill.steps[0],
                {"tool": "browsermcp/browser_navigate", "args": {"url": None}},
            )

    async def test_browser_step_retries_after_connection_error(self) -> None:
        skill = self._browser_skill()
        hub = AsyncMock()
        ...
        with patch(
            "skills.sequence_skill.ensure_browser_mcp_connected",
