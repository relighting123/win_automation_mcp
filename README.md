...
### 실행 중 제어 오버레이 (Windows)

`/analyze` 실행 시 대상 프로그램 상단에 **Chrome 자동화 테스트** 스타일의 제어 HUD가 표시됩니다.

- 상단 배너: `자동화 소프트웨어가 제어 중` (Chrome 인포바와 유사)
- 대상 창 주변: 둥근 글로우 테두리로 자동화 대상 강조
- 컨트롤: **⏸/▶** 일시정지·재개, **⏭** 현재 스킬 스킵, **■** 전체 중지
- `auto` / `semi` / `manual` 모든 모드에서 동작 (Windows + Tkinter 환경)

대상 앱이 연결되기 전(계획 단계)에도 오버레이가 준비되며, 앱 윈도우가 보이면 자동으로 상단에 붙습니다.
...
```bash
# HTTP 방식 (기본)
python mcp_server.py --transport http --host 127.0.0.1 --port 8001 --path /mcp

# SSE 방식 (파일 변경 시 자동 재시작)
python mcp_server.py --transport sse --port 8001 --reload

# 로그 레벨 지정
python mcp_server.py --log-level DEBUG
```

Claude Desktop, Cursor 등 MCP 클라이언트에서 `http://localhost:8001/mcp`로 연결할 수 있습니다.
